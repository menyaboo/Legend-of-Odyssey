...
const SandSection = (): ReactNode => (
    <section className={styles.section} style={{backgroundImage: 'url(/images/используемое/bg-section-2_new.png)'}}>
        <ContainerComponent className={styles.container}>
            <h1>{"Explore the world's ecosystem"}</h1>
            <div className={styles.maps}>
                {[...Array(3)].map((_el, index) =>
                    <Image key={index} src="/images/используемое/гифки/map.gif" alt="map" width={450} height={300}/>
                )}
            </div>
        </ContainerComponent>
    </section>
